...
class EnrollmentCourseListView(generics.ListAPIView):
    permission_classes = [permissions.IsAuthenticated]
    pagination_class = CustomPageNumberPagination
    serializer_class = CourseSerializer
    ...
    def get_queryset(self):
        ...
        for enroll in enrollmentList:
            course_id_list.add(enroll.enrollDate.course_id)
        
        course_id_list = list(course_id_list)
        return Course.objects.filter(id__in=course_id_list)
