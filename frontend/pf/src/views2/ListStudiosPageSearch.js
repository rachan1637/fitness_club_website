...
function ListStudiosPageSearch() {
  ...
  const getStudiosSearch = async (page, inputVal, selectedName, selectedAmenities, selectedClasses, selectedCoaches) => {
    await api
      .get(`http://localhost:8000/studios/list_studios_search/?page=${page}&search=${inputVal}&name=${selectedName}&amenities__Amname=${selectedAmenities}&classes__classNames=${selectedClasses}&coaches__coachNames=${selectedCoaches}`)
      .then((response) => {
        setStudios(response.data.results);
        setPage(page);
      })
...
// amenitiesList = amenitiesList.sort()
  ...
  return (
    <>
      ...
      <button className="border-2 border-black px-2 py-1 ml-auto rounded-lg" onClick = {() => getStudiosSearch(1, inputValv.current.value, selectedName.current.value, selectedAmenities.current.value, selectedClasses.current.value, selectedCoaches.current.value)}>Search</button>
      ...
      {/* <input type="search" placeholder="Filter studio name" value={filterName} onChange={(e) => getStudios(page, setfilterName(e.target.value))} />
      <input type="search" placeholder="Filter studio amenities" value={filterAmeneties} onChange={(e) => getStudios(page, setfilterAmeneties(e.target.value))} />
      <input type="search" placeholder="Filter studio classes" value={filterClass} onChange={(e) => getStudios(page, setfilterAmeneties(e.target.value))} />
      <input type="search" placeholder="Filter studio coaches" value={filterCoach} onChange={(e) => getStudios(page, setfilterCoach(e.target.value))} /> */}
        <button className="border-2 border-black px-2 py-1 ml-auto rounded-lg" onClick={() => getStudiosSearch(page - 1, inputValv.current.value, selectedName.current.value, selectedAmenities.current.value,selectedClasses.current.value, selectedCoaches.current.value)}>
          Previous
        </button>
        <button className="border-2 border-black px-2 py-1 mr-auto rounded-lg"  onClick={() => getStudiosSearch(page + 1, inputValv.current.value, selectedName.current.value, selectedAmenities.current.value,selectedClasses.current.value, selectedCoaches.current.value)}> Next </button>
      </div>
    </>
  );
}
